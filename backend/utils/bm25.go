...
func UpdateBM25Stats(db *sql.DB, userID int64) error {
	log.Printf("Updating BM25 statistics for user %d...", userID)
	...
	if _, err = tx.Exec("DELETE FROM bm25_stats WHERE user_id = ?", userID); err != nil {
	...
	if len(termDocFreq) > 0 {
		...
		args := make([]interface{}, 0, len(termDocFreq)*4)

		for term, docFreq := range termDocFreq {
			values = append(values, "(?, ?, ?, ?)")
			args = append(args, userID, term, docFreq, termTotalFreq[term])
		}

		query := fmt.Sprintf(
			"INSERT INTO bm25_stats (user_id, term, doc_freq, total_freq) VALUES %s",
			strings.Join(values, ","),
		)
	...
	log.Printf("BM25 stats updated for user %d: %d documents, %.2f avg length, %d unique terms",
		userID, totalDocs, avgDocLength, len(termDocFreq))
